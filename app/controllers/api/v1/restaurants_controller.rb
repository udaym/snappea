  ...
  PER_PAGE = 10
	...
	def items
		if @restaurant.present?
			items = @restaurant.products.paginate(:page => params[:page], :per_page => PER_PAGE)
			if items.present?
				items_json = []
				items.each do |item|
					items_data = {}
				...
				render json: {status: true, products: items_json}, status: 200
			else
    ...
    def authenticate_token
    	if params[:api].present?
      	@current_app_client = ::Client.find_by(uuid: params[:api])
